...
public static class CollisionDetection
{
    ...
    {
        Sphere s = shape1 as Sphere;
        AABB aABB = shape2 as AABB;
        Vector3 closestPoint = new Vector3();

        for (int i = 0; i < 3; i++)
        {
            float pos = s.transform.position[i];
            closestPoint[i] = pos;
            float min = aABB.transform.position[i] - aABB.halfwidths[i];
            float max = aABB.transform.position[i] + aABB.halfwidths[i];
            if (pos < min)
            {
                closestPoint[i] = min;
            }
        ...
        penetration = s.Radius - (s.Center - closestPoint).magnitude;
        normal = (s.Center - closestPoint).normalized;
    }

    public static void TestSphereOBB(PhysicsCollider shape1, PhysicsCollider shape2, out Vector3 normal, out float penetration)
    {
        Sphere s = shape1 as Sphere;
        OBB oBB = shape2 as OBB;
        Vector3 closestPoint = new Vector3();

        var oBBmatrix = oBB.transform.localToWorldMatrix;
        Vector3 sLocal = oBBmatrix.inverse * new Vector4(s.transform.position.x, s.transform.position.y, s.transform.position.z, 1f);
        Vector3 oBBLocal = oBBmatrix.inverse * new Vector4(oBB.transform.position.x, oBB.transform.position.y, oBB.transform.position.z, 1f);

        for (int i = 0; i < 3; i++)
        {
            float pos = sLocal[i];
            closestPoint[i] = pos;
            float min = oBBLocal[i] - 0.5f;
            float max = oBBLocal[i] + 0.5f;
            if (pos < min)
            {
                closestPoint[i] = min;
            }
        ...
        difference = oBBmatrix * new Vector4(difference.x, difference.y, difference.z, 0);
